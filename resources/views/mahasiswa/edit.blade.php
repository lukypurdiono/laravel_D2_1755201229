...
@section('bread2','Edit Data Mahasiswa')
@section('content')
    ...
    <form action="{{ route('mhs.update', $mhs->nim) }}" method="POST">
        ...
        <div class="form-group row">
            ...
            <input type="text" name="nim" class="form-control" id="nim" placeholder="Nomor Induk Mahasiswa" value="{{ $mhs->nim }}" readonly>
                @error('nim')<small id="nim" class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
        </div>
            ...
            <label for="nim" class="col-sm-12">Nama Lengkap</label>
                <div class="col-sm-5">
                    <input type="text" name="nama_lengkap" class="form-control" id="nama_lengkap" placeholder="Masukan nama dengan benar" value="{{ $mhs->nama_lengkap }}">
                        @error('nama_lengkap')
                            <small id="nama_lengkap" class="form-text textdanger">{{ $message }}</small>
                        @enderror
                </div>
        </div>
